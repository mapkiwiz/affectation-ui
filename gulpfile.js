...
var paths = {
  scripts: [yeoman.app + '/scripts/**/*.js', yeoman.app + '/map/scripts/**/*.js'],
...
gulp.task('build', ['clean:dist'], function (cb) {
  runSequence(['images', 'copy:template', 'copy:data', 'copy:extras', 'copy:fonts', 'client:build'], cb);
});
...
gulp.task('maven:install', ['maven:copy'], function() {
  return gulp.src('.')
    .pipe(maven.install({
        'config': {
          'groupId': 'com.github.mapkiwiz.examples',
          'type': 'jar',
          'buildDir': 'target'
        }
      })
